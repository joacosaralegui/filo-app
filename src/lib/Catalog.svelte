...
>
  ...
  <div class="flex flex-col gap-[18px]">
    {#each courses as c (c.id)}
      ...
      <button
        class="relative flex w-full cursor-pointer flex-col overflow-hidden rounded-[4px] halftone-surface-subtle bg-surface text-left [font-family:inherit] active:scale-[0.995]"
      ...
      >
        ...
        {#if num != null && c.total}
          ...
          <div
            class="absolute inset-x-0 bottom-0 h-[4px] overflow-hidden bg-accent-3/20"
            role="progressbar"
            aria-valuenow={pct}
            aria-valuemin="0"
            aria-valuemax="100"
          ...
          >
            <div class="h-full bg-accent-3" style="width: {pct}%"></div>
          </div>
        {/if}
      </button>
    {/each}
